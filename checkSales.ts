...
const buildMessage = (sale: any) => (
  ...
  .setImage(sale.asset.image_url)
	...
	.setFooter('Created on OpenSea', 'https://files.readme.io/566c72b-opensea-logomark-full-colored.png')
)

async function main() {
  const channel = await discordSetup();
  const seconds = process.env.SECONDS ? parseInt(process.env.SECONDS) : 1_800;
